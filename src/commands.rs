...
pub fn init(args: &[String]) -> Result<()> {
    ...
    let project_root = env::current_dir()?.to_str().unwrap().to_string() + "/" + name;
    ...
    fs::create_dir(name)?;
    env::set_current_dir(Path::new(&format!("./{name}")))?;
...
pub fn fmt(args: &[String]) -> Result<()> {
    ...
    let mut src_files: Vec<String> = vec![String::from(format!("-style={style}"))];
    if in_place {
        src_files.push(String::from("-i"));
    }
    collect_src_files(
        PathBuf::from(format!("{}/src", project_root)),
        &mut src_files,
    )?;
    collect_src_files(
        PathBuf::from(format!("{}/test", project_root)),
        &mut src_files,
    )?;
...
pub fn check() -> Result<()> {
    ...
    if config.cppcheck {
        ...
        collect_src_files(
            PathBuf::from(format!("{}/src", config.project_root.as_str())),
            &mut src_files,
        )?;
        collect_src_files(
            PathBuf::from(format!("{}/test", config.project_root.as_str())),
            &mut src_files,
        )?;
    ...
    if config.clang_tidy {
        ...
        print!(
            "\x1b[1;32mclang-tidy passed\x1b[0m ({}s)\n",
            compile_time.as_secs()
        );
    }
    ...
    let total_time = start.elapsed();
    print!(
        "\x1b[1;32mAll checks passed\x1b[0m ({}s)\n\n",
        total_time.as_secs()
    );
    Ok(())
}
...
pub fn branch(args: &[String]) -> Result<()> {
    ...
    for (flag, _) in arg_map {
        ...
        Command::new("git")
            ...
            .output()?;
        Command::new("git")
            .args(["branch", "--set-upstream-to=origin/main", flag])
            .stdout(Stdio::inherit())
            .stderr(Stdio::inherit())
            .output()?;
    }
        ...
        match key {
            ...
            _ => {
                return Err(Error::new(
                    ErrorKind::Other,
                    format!(
                        "\x1b[31mpackage not found:\x1b[0m{}\n\tsee 'gojo install --list'\n",
                        key
                    ),
                ));
            }
...
fn collect_src_files(path: PathBuf, src_files: &mut Vec<String>) -> Result<()> {
    let config_result = config_read();
    let mut config = config_default();
    if config_result.is_some() {
        config = config_result.unwrap();
...
fn config_default() -> GojoConfig {
    let project_root = String::from(env::current_dir().unwrap().to_str().unwrap());
